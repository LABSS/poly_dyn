...
globals [init_data global_gender global_age_class global_country_area global_s mu_theta_interact]
...
to setup
  ...
  set mu_theta_interact csv:from-file "mu_theta_interact_separated.csv"
...
to interact [u v]
    ...
    let start_area [s_country_area] of u
    let start_gender [s_gender] of u
    let start_age [s_age_class] of u
    let target_area [s_country_area] of v
    let target_gender [s_gender] of v
    let target_age [s_age_class] of v

    let q 1
    let valore -1
    while [q < 901]              ;;; oppure while [not empty? mu_theta_interact]   => MA dà ERRORE!!!!!!!!!!
       [let row item q mu_theta_interact
        if (start_area = item 2 row) and (start_gender = item 3 row)
        and (start_age = item 4 row) and (target_area = item 5 row)
        and (target_gender = item 6 row) and (target_age = item 7 row)
        [
          set valore item 1 row
        ]
        set q (q + 1)
        ]
    set mu_w mu_w + (1 - mu_w) * valore   ;; assegna mu_w
